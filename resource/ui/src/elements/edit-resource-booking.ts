...
export class EditResourceBooking extends ScopedElementsMixin(LitElement) {
  ...
  firstUpdated() {
    this._avatar = this.resourceBooking?.fields['avatar'];
  ...
  fireSaveResourceBooking() {
    ...
    this.dispatchEvent(
      new CustomEvent('save-resource-booking', {
        detail: {
          resourceBooking,
        },
        bubbles: true,
  ...
  renderField(fieldName: string) {
    return html`
      <mwc-textfield
        id="resource-booking-field-${fieldName}"
        outlined
        required
        autoValidate
        validationMessage="This field is required"
        .label=${fieldName}
        .value=${this.resourceBooking?.fields[fieldName] || ''}
        ...
        <div class="column">
          ...
          <div class="row" style="justify-content: center; margin-bottom: 8px; align-self: start;" >
            ...
            <mwc-textfield
              ...
              outlined
              label="Nickname"
              .value=${this.resourceBooking?.nickname || ''}
