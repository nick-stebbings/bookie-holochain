...
export class SearchAgent extends ScopedElementsMixin(LitElement) {
  ...
  private _knownResource = new StoreSubscriber(
    this,
  ...
  private get _filteredAgents(): Array<AgentBookableResource> {
    let filtered = Object.entries(this._knownResource.value)
      ...
      .map(([agentPubKey, resourceBooking]) => ({ agentPubKey, resourceBooking }));
  ...
  async searchAgents(nicknamePrefix: string): Promise<void> {
    ...
    await this.store.searchResource(nicknamePrefix);
  }
  ...
  onUsernameSelected(agent: AgentBookableResource) {
    ...
    if (agent) {
      ...
      if (this.clearOnSelect) {
        ...
        this._textField.value = agent.resourceBooking.nickname;
      }
      this._overlay.close();
    }
  }
                  ...
                  ${this._filteredAgents.map(
                    ...
                    >
                      ...
                      <span style="margin-left: 8px;"
                        >${agent.resourceBooking.nickname}</span
                      >
                    </mwc-list-item>`
                  )}
                </mwc-list>
